...
open class ImageSlideShowViewController: UIPageViewController, UIPageViewControllerDataSource, UIPageViewControllerDelegate
{
    ...
    var descView:UIView=UIView()
    var textViewDesc: UITextView = UITextView()
	open var slides: [ImageSlideShowProtocol]?
    open var imgDescription:NSMutableArray=NSMutableArray()
    ...
    {
        return imgDescription.object(at: currentIndex) as! String
    }
	...
	override open func viewDidLoad()
	{
       ...
       let leftButton: UIBarButtonItem = UIBarButtonItem(customView: buttonleft)
        ...
        buttonRight.contentMode = .scaleAspectFit
        buttonRight.setImage(UIImage(named:"share"), for:  UIControl.State.normal)
        buttonRight.addTarget(self, action: #selector(shareCurrentImage), for: UIControl.Event.touchUpInside)
        let rightButton: UIBarButtonItem = UIBarButtonItem(customView: buttonRight)
        //UIBarButtonItem(barButtonSystemItem: .done, target: self, action: #selector(dismiss(sender:)))
    ...
    @objc func backClicked()
   {
    ...
    func setFooterView()
    {
       // descView.isHidden=true
        descView.frame = CGRect(x: 0, y: self.view.frame.height-150, width: self.view.frame.width, height: 150)

        textViewDesc.frame = CGRect(x: 10, y: 0, width: self.descView.frame.width-20, height: 128)
        textViewDesc.isEditable=false
       // textViewDesc.isUserInteractionEnabled=false
        textViewDesc.textColor=UIColor.white
        textViewDesc.font=UIFont(name:"Roboto", size: 15)
        textViewDesc.backgroundColor=UIColor.clear
        descView.backgroundColor=UIColor.black
        descView.addSubview(textViewDesc)
        descView.bringSubviewToFront(textViewDesc)
        self.view.addSubview(descView)
        self.view.bringSubviewToFront(descView)
    }
    
    @objc func shareCurrentImage()
    {
        currentSlide?.image(completion:{ (image, error) -> Void in
	...
	private func slideViewController(forPageIndex pageIndex: Int) -> ImageSlideViewController?
	{
		if let slides = slides, slides.count > 0
		{
            ...
            let description = imgDescription.object(at: pageIndex) as! String
			...
			else
			{
                ...
                controller.imgDescrition = description
...
UIView.animate(withDuration: 0.3,
	...
	private func updateSlideBasedUI()
	{
		if let title = currentSlide?.title
		{
            ...
            textViewDesc.text = currentDescription
            if currentDescription.count<40
            {
