...
+ (id)fromJSONDataForAlbum:(NSData *)data
{
    NSString *jsonString = [[[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding] autorelease];
    id jsonObject = [jsonString JSONValue];
    return [self deserializeJSONFromAlbum:jsonObject];
...
- (NSString *) convertProperty:(NSString *)propertyName andClassName:(NSString *)className {
    ...
    {
       //  printf(@"...propertyName",propertyName);
...
+ (id) deserializeJSONFromAlbum:(id)jsonObject
{
    id result = nil;
    return result;
}
