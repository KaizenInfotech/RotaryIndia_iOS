...
-(void)viewWillAppear:(BOOL)animated {
    ...
    self.searchBar.tintColor = [UIColor colorWithRed:250.0/255.0 green:104.0/255.0 blue:67.0/255.0 alpha:1.0];
...
-(void)methodForAddLeft
{
    UIButton *btnLib = [UIButton buttonWithType:UIButtonTypeCustom];
    [btnLib setImage:[UIImage imageNamed:@"backArrow.png"] forState:UIControlStateNormal];
    btnLib.frame = CGRectMake(0, 0, 25, 18);
    btnLib.showsTouchWhenHighlighted=YES;
    [btnLib addTarget:self action:@selector(btnBackToGoBackClickEvent) forControlEvents:UIControlEventTouchUpInside];
    UIBarButtonItem *barButtonItem2 = [[UIBarButtonItem alloc] initWithCustomView:btnLib];
    NSMutableArray *arrRightBarItems = [[NSMutableArray alloc] init];
    [arrRightBarItems addObject:barButtonItem2];
    self.navigationItem.leftBarButtonItems=arrRightBarItems;
}
- (void)btnBackToGoBackClickEvent
{
...
{
    self.googleAutoCompleteTableView.hidden=YES;
    [self.searchBar resignFirstResponder];
    ...
    [[ABCGooglePlacesAPIClient sharedInstance] retrieveJSONDetailsAbout:autoCompleteResult.placeID withCompletion:^(NSDictionary *placeInformation, NSError *error) {
        ...
        NSString *strGetFormattedAddress=[placeInformation objectForKey:@"formatted_address"];
        [self.delegate searchViewController:self didReturnPlace:place];
        
        [[NSUserDefaults standardUserDefaults] setValue:strGetFormattedAddress forKey:@"session_setGetFormattedAddress"];
        [[NSUserDefaults standardUserDefaults] setValue:[@(place.location.coordinate.latitude) stringValue] forKey:@"session_setGetLatitude"];
        [[NSUserDefaults standardUserDefaults] setValue:[@(place.location.coordinate.longitude) stringValue] forKey:@"session_setGetLongitude"];
