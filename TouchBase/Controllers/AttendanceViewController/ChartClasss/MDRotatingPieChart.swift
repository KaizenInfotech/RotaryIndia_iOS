...
struct Properties {
    ...
    var smallRadius:CGFloat = 30
    //biggest of both radius
    var bigRadius:CGFloat = 77
    //value of the translation when a slice is openned
    var expand:CGFloat = 15
    
    //label format in slices
    var displayValueTypeInSlices:DisplayValueType = .value
    ...
    init() {
        ...
        let vartGetStatus = UserDefaults.standard.value(forKey: "session_setGetTodayReviews") as! String
        if(vartGetStatus == "today")
        {
            smallRadius = 30
            bigRadius = 77
            expand = 15
        }
        else  if(vartGetStatus == "reviews")
        {
            smallRadius = 0
            bigRadius = 95
            expand = 15
...
class MDRotatingPieChart: UIControl {
    ...
    func createLabel(_ angleSum:CGFloat, slice:Slice) -> UILabel {
        ...
        let WageConversion = (label.text! as NSString).floatValue
        
        
        
        
        //let varTemp:String = label.text!
        //let  sampleValue:Float = varTemp
        //let varGetNewValues = (sampleValue.cleanValue)
        
        
        label.text = String(WageConversion)
        
        // var varGetValue:CGFloat = (label.text) as CGFloat
