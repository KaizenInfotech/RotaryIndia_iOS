...
#define IS_OS_8_OR_LATER ([[[UIDevice
#import "UIView+Toast.h"
...
int counts=1;
@interface BRPreviewViewController ()<UICollectionViewDataSource, UICollectionViewDelegate,BRPhotoPickerControllerDelegate,UITextFieldDelegate,PECropViewControllerDelegate,MONActivityIndicatorViewDelegate>
{
    MONActivityIndicatorView *indicatorView;
    UIImageView *bg;
    CGSize cellSize;
}
//@property (weak, nonatomic) IBOutlet UIButton *buttonOpticity;
...
- (IBAction)donePressed:(UIButton *)sender {
    ...
    NSString *strCountImages = [[NSUserDefaults standardUserDefaults]stringForKey:@"ImageCountFiveOrNot"];
    NSLog(@"More then Five Images %@",strCountImages);
    NSLog(@"fgdfgdf %d",self.selectedAssetsArray.count);
    NSInteger a = self.selectedAssetsArray.count + [strCountImages integerValue];
    NSLog(@"fgdfgdf %d",a);
    if(a>5)
    {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Rotary India"
                                                        message:@"Maxium 5 photos are allowed."
                                                       delegate:self
    ...
    int varIfGotoInside=0;
    
    //--------
    for(int i=0;i<count;i++)
    {
       // [MBProgressHUD showHUDAddedTo:self.view animated:YES];
        BR_AssetsInfo *assetsInfo=[[BR_AssetsInfo alloc]init];
        UIImage *images=[[UIImage alloc]init];
        //selectedAssetsArray---------------------DPK
        assetsInfo=[self.selectedAssetsArray objectAtIndex:i];
        images=assetsInfo.originalImage;
        // [self saveImageToServer:images];
        UIImage *originalImage = images;
        NSData *imgData = UIImageJPEGRepresentation(originalImage, 0.15f); //1 it represents the quality of the image.
        NSLog(@"Size of Image(bytes):%d",[imgData length]); //---1 kb to 1000byte
        NSLog(@"Size of Image(kb):%d",[imgData length]/1000); //---1 kb to 1000byte
        NSLog(@"Size of Image(bytes):%d",([imgData length]/1000)/1000); //---1 kb to 1000byte
        ...
        /*
        if((float)imgData.length/1024.0f/1024.0f>1)
        {
            
            varIfGotoInside=1;
            dispatch_async(dispatch_get_main_queue(), ^{
                [self methodForProgressBarHide];
                [self.view makeToast:[NSString stringWithFormat:@"Image %d  size %f is too high, maximum 1 MB image can be added",i+1,(float)imgData.length/1024.0f/1024.0f] duration:10 position:CSToastPositionTop];
            });
            break;
        }
        */
    ...
    else
    {
        ...
        for(int i=0;i<count;i++)
        {
            ...
            assetsInfo=[self.selectedAssetsArray objectAtIndex:i];
            images=assetsInfo.originalImage;
            NSString *strCaption = assetsInfo.caption;
            NSLog(@"%@ Caption",strCaption);
            if([strCaption isEqual:nil] || [strCaption isEqual:NULL] || strCaption.length == 0 )
            {
                strCaption=@"";
            }
            counts = i;
            counts=counts+1;
            NSLog(@"%d",counts);
            NSLog(@"%d",_selectedAssetsArray.count);
            [self saveImageToServer:images param:strCaption];
        }
...
-(void)saveImageToServer:(UIImage *)PhotoImage param:(NSString*)stringCaption
{
    ...
    if ([returnData length] > 0)
    {
       ...
       // NSLog(@"%d,dfdsfsdfsdfsd",_selectedAssetsArray.count);
        ...
        if(_selectedAssetsArray.count==counts)
        {
    ...
    /*
    [NSURLConnection sendAsynchronousRequest:request queue:queue  completionHandler:^(NSURLResponse *response, NSData *data, NSError *errorReturned)
     {
         if (errorReturned)
         {
             //...handle the error
             
             UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"TouchBasE"
  ...
  [self methodForProgressBarHide];
         ...
         if ([data length] > 0)
         {
             ...
             [self.view makeToast:@"Image added successfully" duration:3 position:CSToastPositionCenter];
             NSArray* latestLoans = [json objectForKey:@"LoadImageResult"];
             NSLog(@"LoadImageResult: %@", latestLoans);
             // NSString *returnString = [latestLoans valueForKey:@"stataus"];
             NSLog(@"==> sendSyncReq returnString: %@", urlString);
